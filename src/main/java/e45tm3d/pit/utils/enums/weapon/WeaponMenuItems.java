...
public enum WeaponMenuItems {
    ...
    public ItemStack getItemStack() {
        try {
            String material = Yaml.WEAPON.getConfig().getString("menu.items." + getType() + ".material");
            if (material == null) {
                // 使用默认材质作为后备
    ...
    private ItemStack setupItemMeta(ItemStack item) {
        try {
            ...
            if (meta != null) {
                String displayName = Yaml.WEAPON.getConfig().getString("menu.items." + getType() + ".name");
                if (displayName != null) {
                    meta.setDisplayName(displayName.replaceAll("&", "§"));
                }
                
                ArrayList<String> lores = new ArrayList<>();
                if (Yaml.WEAPON.getConfig().contains("menu.items." + getType() + ".lore")) {
                    java.util.List<String> loreList = Yaml.WEAPON.getConfig().getStringList("menu.items." + getType() + ".lore");
