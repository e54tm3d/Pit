...
public enum BuffItems {
    ...
    private ItemStack setupItemMeta(ItemStack item) {
        try {
            ...
            if (meta != null) {
                String displayName = Yaml.BUFF.getConfig().getString("equiped." + getType() + ".name");
                if (displayName != null) {
                    meta.setDisplayName(displayName.replaceAll("&", "§"));
                }
                
                ArrayList<String> lores = new ArrayList<>();
                if (Yaml.BUFF.getConfig().contains("equiped." + getType() + ".lore")) {
                    java.util.List<String> loreList = Yaml.BUFF.getConfig().getStringList("equiped." + getType() + ".lore");
                    if (loreList != null && !loreList.isEmpty()) {
                        for (String lore : loreList) {
                            String price = Yaml.BUFF.getConfig().getString("equiped." + getType() + ".price");
                            if (price != null) {
    ...
    public static BuffItems getItem(String item) {
        if (item == null || item.isEmpty()) {
            return null;
        }
        String str = item.toUpperCase();
        for (BuffItems itemType : BuffItems.values()) {
